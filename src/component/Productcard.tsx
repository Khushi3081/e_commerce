...
const Productcard: React.FC<{ data: ProductDetailsPageType[] }> = ({data}) => {
 
  return (
    <Row lg={4}>
    {data &&
      data.map((product) => {
        const { uuid,title,description, images,price,Quantity,category } =
          product;
        return (
          <Col className="d-flex my-3">
            <Card className="flex-fill" key={uuid}>
