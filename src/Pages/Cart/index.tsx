...
const Cart = () => {
  ...
  const [payment, setPayment] = useState<{ [key: string]: boolean }>({});
  ...
  const handlePayment = async (cartProduct: cartData) => {
    ...
    if (response?.data.responseType === ResponseType.Success) {
      setPayment({ [response.data.data]: true });
      getCartData();
    }
  };
  useEffect(() => {
  ...
  return (
    <div>
      {cartData && cartData.length > 0 ? (
        <CartProductcard
          data={cartData}
          addQuantity={(uuid, quantity, price) =>
            handleAddQuantity(uuid, quantity, price)
          }
          removeQuantity={(uuid, quantity, price) =>
            handleRemoveQuantity(uuid, quantity, price)
          }
